...
background_paths = [
    ...
    for fname in sorted(os.listdir(fara_carie_dir))[:10]
    if fname.lower().endswith((".jpg", ".png", ".jpeg"))
]

# Încarcă imaginile ca obiecte PIL.Image
...
def detect():
    ...
    for crop_path, idx in cropped_paths:
        ...
        explanation_path, predicted_class, confidence = generate_gradcam(
            crop_image, cnn_model,
            region_id=os.path.splitext(os.path.basename(crop_path))[0]
        )
        explanation_url = f"/explanations/{os.path.basename(explanation_path)}"
        gradcam_image_urls.append(explanation_url)

        # SHAP explanation
        shap_path = generate_shap(
            crop_image, shap_explainer,  # folosești explainer-ul, nu modelul
            region_id=os.path.splitext(os.path.basename(crop_path))[0]
        )

        shap_url = f"/explanations/{os.path.basename(shap_path)}"
